...
type ToolDeps = {
  ...
  getContacts: () => ContactBook | null;
  getStateManager: () => StateManager | null;
  getChatHistory: () => ChatHistoryManager | null;
  getConfig: () => BlueBubblesPluginConfig;
...
function requireClient(deps: ToolDeps): BlueBubblesClient {
  const client = deps.getClient();
  if (!client) {
    throw new Error(
      'BlueBubbles is not connected. Configure the server URL and password in BlueBubbles settings, then reconnect.',
    );
  }
  return client;
}
  ...
  return [
    {
      name: 'list-contacts',
      description:
        'List all saved contacts in the BlueBubbles address book. Returns a mapping of phone numbers/email addresses to display names. Use this to find a person\'s address before sending them a message.',
      inputSchema: {
        type: 'object',
        properties: {
          search: {
      ...
      execute: async (input: unknown) => {
        try {
          ...
          if (!contacts) return { error: 'Contacts not initialized' };
...

    {
      name: 'save-contact',
      description:
        'Save or update a contact name for a phone number or email address. This maps an address to a friendly display name used throughout the messaging interface.',
      inputSchema: {
        type: 'object',
        properties: {
          address: {
      ...
      execute: async (input: unknown) => {
        try {
          ...
          if (!contacts) return { error: 'Contacts not initialized' };
          const data = input as { address: string; name: string };
...

    {
      name: 'list-chats',
      description:
        'List recent iMessage and SMS conversations. Returns chats sorted by most recent activity, including participant names, last message preview, and unread status. Use this to find a specific conversation before fetching its messages or sending a reply.',
      inputSchema: {
        type: 'object',
        properties: {
          limit: {
      ...
      execute: async (input: unknown) => {
        try {
          ...
          let chats = bbChats.map((c) =>
            normalizeChat(c, contacts ? (addr) => contacts.resolve(addr) : undefined),
          );
          if (data.search) {
            const term = String(data.search).toLowerCase();
            chats = chats.filter(
...

    {
      ...
      description:
        'Fetch message history from a specific conversation. Returns messages in chronological order with sender names, timestamps, reactions, and reply threading. Use the chat GUID from list-chats to identify the conversation.',
      inputSchema: {
        type: 'object',
        properties: {
          chatGuid: {
      ...
      execute: async (input: unknown) => {
        try {
          const client = requireClient(deps);
          const contacts = deps.getContacts();
          const data = input as { chatGuid: string; limit?: number; offset?: number };
          const limit = Math.min(Number(data.limit) || 50, 100);
          ...
          const allNormalized = bbMessages
            .map((m) =>
              normalizeMessage(
                ...
                contacts ? (addr) => contacts.resolve(addr) : undefined,
              ),
            )
            .reverse();
...

    {
      ...
      execute: async (input: unknown) => {
        try {
          ...
          const data = input as { chatGuid: string; text: string; replyToGuid?: string };

          const results = await client.sendChunkedText(
            data.chatGuid,
            data.text,
            chunkConfig.maxLength,
            { replyToGuid: data.replyToGuid },
          );
...

    {
      ...
      execute: async (input: unknown) => {
        try {
          const client = requireClient(deps);
          const data = input as { chatGuid: string; messageGuid: string };
...

    {
      ...
      execute: async (input: unknown) => {
        try {
          const client = requireClient(deps);
          const data = input as { chatGuid: string; messageGuid: string; reaction: string };
...

    {
      ...
      execute: async (input: unknown) => {
        try {
          const client = requireClient(deps);
          const data = input as { chatGuid: string; messageGuid: string; text: string };
...

    {
      ...
      execute: async (input: unknown) => {
        try {
          const client = requireClient(deps);
          const stateManager = deps.getStateManager();
          const data = input as { chatGuid: string };
...

    {
      ...
      description:
        'Get the current BlueBubbles connection status and server information. Useful for checking if the messaging service is connected and working.',
      inputSchema: {
        type: 'object',
        properties: {},
      },
