import type { NormalizedMessage, NormalizedReaction, ReactionType } from '../shared/types.js';
...
export function isReactionMessage(raw: any): boolean {
...
export function extractReactionInfo(raw: any): { type: string; isRemoval: boolean } | null {
  const assocType = raw.associatedMessageType;
  if (typeof assocType === 'string') {
...
export function extractTargetGuid(associatedMessageGuid: string): string {
