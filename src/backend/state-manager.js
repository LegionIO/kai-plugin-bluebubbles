...
export class StateManager {
    ...
    constructor(stateApi, uiApi, notificationsApi, log) {
    ...
    setTypingIndicator(chatGuid, isTyping) {
    ...
    addReaction(targetMessageGuid, reaction) {
        const idx = this.state.activeChatMessages.findIndex((m) => m.guid === targetMessageGuid);
        if (idx < 0)
            return;
        const msg = { ...this.state.activeChatMessages[idx] };
        const existing = msg.reactions.findIndex((r) => r.type === reaction.type && r.sender === reaction.sender);
        if (existing >= 0)
            return;
    ...
    removeReaction(targetMessageGuid, reaction) {
        const idx = this.state.activeChatMessages.findIndex((m) => m.guid === targetMessageGuid);
        if (idx < 0)
            return;
        const msg = { ...this.state.activeChatMessages[idx] };
        msg.reactions = msg.reactions.filter((r) => !(r.type === reaction.type && r.sender === reaction.sender));
        this.state.activeChatMessages[idx] = msg;
        this.state.activeChatMessages = [...this.state.activeChatMessages];
        this.stateApi.set('activeChatMessages', this.state.activeChatMessages);
    }
    ...
    setAIReplyProcessing(chatGuid, processing) {
    ...
    pushFullState() {
        this.stateApi.replace(this.state);
    }
}
